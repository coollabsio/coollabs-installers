...
async function configureDocker(install) {
    ...
    if (docker.code !== 0) {
        ...
        if (dockerMajorVersion !== '20') {
            if (install) {
                console.log('Updating Docker to v20...')
                installDocker()
            }
        }
    }
...
async function installDocker() {
    ...
    shell.exec('apt-get install -y apt-transport-https ca-certificates curl gnupg-agent software-properties-common bind9-dnsutils git')
...
async function configureSwarm() {
    if (!publicAddress) {
        const addresses = shell.exec('hostname --all-ip-addresses || hostname -I').stdout.replace(/\s+/g, ' ').trim().split()
        if (addresses.length === 1) {
                ...
                .prompt([
                    {
                        type: 'list',
                        name: 'ipaddress',
                        message: 'Which IP address to use for Docker Swarm? Should be reachable if other servers need to reach this one - so public IP please!',
                        choices: addresses,
                        filter: function (val) {
                            if (val) {
            ...
            publicAddress = answers.ipaddress
        }
    }
    console.log('Initiating Docker Swarm...')
...
function saveConfig() {
    fs.writeFileSync(confFile, JSON.stringify(answers))
}

async function dns() {
    const isDnsSet = await inquirer
        .prompt([
            {
                type: 'confirm',
                name: 'dns',
                message: `Please make sure the DNS name you set before, is pointing to the server's public IP Address.

Add an A record pointing to your server's public IP. 
Your public IP: ${publicAddress}
...
Are you ready to continue?`,
    ...
    if (nslookup !== publicAddress) {
        ...
        answers.general = { ...answers.general, ...isDnsSet }
    }
}
...
async function startQuestioningYourLife(express) {
    if (fs.existsSync(confFile)) configJson = fs.readFileSync(confFile).toString('utf8')
    if (express) {
        await generalQuestions()
        await configureDocker(true)
        ...
        shell.exec(`docker network create ${docker.networkName} --driver overlay`)

        console.log('\n## MongoDB related questions')

        const installMongodb = await inquirer
            .prompt([
                {
                    type: 'confirm',
                    name: 'required',
        ...
        answers.mongodb.required = installMongodb.required
...
async function main() {
    ...
    const initialAnswers = await inquirer
        .prompt([
            {
                type: 'confirm',
                name: 'express',
                message: `I will delete/install/update a lot of stuffs on your operating system automagically! By continuing you accept this!
