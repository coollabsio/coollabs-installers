...
function installCoolify($domain, $mongodbRootPassword, $mongodbUser, $mongodbPassword, $mongodbDB)
{
    ...
    $starttime = microtime(true);
    system('docker stack ps coollabs-mongodb >/dev/null && docker stack rm coollabs-mongodb >/dev/null && sleep 10');
    system('docker volume inspect coollabs-mongodb-data >/dev/null && docker volume rm -f coollabs-mongodb-data >/dev/null');
    $endtime = microtime(true);
    $timediff = round(($endtime - $starttime) * 1000);
    dump_chunk("$timediff ms<br/>");
